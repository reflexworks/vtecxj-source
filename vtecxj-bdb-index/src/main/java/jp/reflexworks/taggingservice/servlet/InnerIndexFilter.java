 ...
 * BDBインデックスサーブレットフィルタ
 */
public class InnerIndexFilter implements Filter, ReflexServletConst {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		// リクエスト・レスポンスをラップ
		InnerIndexRequest req = new InnerIndexRequest(httpReq);
		InnerIndexResponse resp = new InnerIndexResponse(req, httpResp);

		RequestInfo requestInfo = req.getRequestInfo();

		try {
			if (!BDBEnvUtil.isRunning()) {
				throw new IllegalStateException("Tagging BDB service (index) is not running.");
			}
	 ...
	 * @param req リクエスト
	 */
	private void checkRequest(InnerIndexRequest req) {
