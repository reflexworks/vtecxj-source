...
/**
 * FullTextIndexBDBサーブレットフィルタ
 */
public class FullTextSearchFilter implements Filter, ReflexServletConst {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		// リクエスト・レスポンスをラップ
		FullTextSearchRequest req = new FullTextSearchRequest(httpReq);
		FullTextSearchResponse resp = new FullTextSearchResponse(req, httpResp);

		RequestInfo requestInfo = req.getRequestInfo();

		try {
			if (!BDBEnvUtil.isRunning()) {
				throw new IllegalStateException("Tagging BDB service (full text search) is not running.");
			}
	 ...
	 * @param req リクエスト
	 */
	private void checkRequest(FullTextSearchRequest req) {
	...
	throws IOException {
		...
		// デバッグモードの場合スタックトレースをログ出力
		if (logger.isDebugEnabled()) {
			// NoEntryException、セッションなしエラー以外を出力
			String msg = e.getMessage();
			if (!(e instanceof NoEntryException) &&
					!(e instanceof IllegalParameterException && msg.indexOf("Session is required.") > -1)) {
				logger.debug(LogUtil.getRequestInfoStr(requestInfo) + e.getClass().getSimpleName(), e);
			}
		}
