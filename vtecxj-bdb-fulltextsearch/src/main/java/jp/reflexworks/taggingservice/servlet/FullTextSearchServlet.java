 ...
 */
public class FullTextSearchServlet extends ReflexServlet {
	...
	throws IOException {
		if (!(httpReq instanceof FullTextSearchRequest)) {
			logger.warn("[doGet] HttpServletRequest is not FullTextSearchRequest. " + httpReq.getClass().getName());
			return;
		}
		if (!(httpResp instanceof FullTextSearchResponse)) {
			logger.warn("[doGet] HttpServletResponse is not FullTextSearchResponse. " + httpResp.getClass().getName());
			return;
		}
		FullTextSearchRequest req = (FullTextSearchRequest)httpReq;
		FullTextSearchResponse resp = (FullTextSearchResponse)httpResp;
		FullTextSearchRequestParam param = req.getRequestType();
		...
		FullTextSearchContext reflexContext = new FullTextSearchContext(serviceName, namespace,
				sid, requestInfo, connectionInfo);
		try {
			int status = HttpStatus.SC_OK;
			Object retObj = null;

			// Feed検索 : GET /b{親キー}?f&{項目名}-ft-{値}&l={件数}&p={カーソル}
			// 件数取得 : GET /b{親キー}?c&{項目名}-ft-{値}&l={件数}&p={カーソル}
			// BDBデータ確認 (管理用) : GET /b/?_list={テーブル名}
			// BDBの統計情報取得 (管理用) : GET /b/?_stats

			if (param.getOption(FullTextSearchRequestParam.PARAM_LIST) != null) {
				// テーブルリスト取得
				if (logger.isTraceEnabled()) {
			...
			} else if (param.getOption(FullTextSearchRequestParam.PARAM_COUNT) != null) {
				// 件数取得
				if (logger.isTraceEnabled()) {
			...
			} else if (param.getOption(FullTextSearchRequestParam.PARAM_FEED) != null) {
				// Feed取得
				if (logger.isTraceEnabled()) {
			...
			} else if (param.getOption(FullTextSearchRequestParam.PARAM_STATS) != null) {
				// BDB環境統計情報取得
				if (logger.isTraceEnabled()) {
			...
			} else if (param.getOption(FullTextSearchRequestParam.PARAM_DISKUSAGE) != null) {
				// ディスク使用率を取得
				if (logger.isTraceEnabled()) {
	...
	throws IOException {
		if (!(httpReq instanceof FullTextSearchRequest)) {
			logger.warn("[doPut] HttpServletRequest is not FullTextSearchRequest. " + httpReq.getClass().getName());
			return;
		}
		if (!(httpResp instanceof FullTextSearchResponse)) {
			logger.warn("[doPut] HttpServletResponse is not FullTextSearchResponse. " + httpResp.getClass().getName());
			return;
		}
		FullTextSearchRequest req = (FullTextSearchRequest)httpReq;
		FullTextSearchResponse resp = (FullTextSearchResponse)httpResp;
		...
		try {
			...
			if (param.getOption(FullTextSearchRequestParam.PARAM_INDEX) != null) {
				// データ更新
				if (logger.isTraceEnabled()) {
			...
			} else if (param.getOption(FullTextSearchRequestParam.PARAM_BACKUP) != null) {
				// BDBバックアップ
				if (logger.isTraceEnabled()) {
	...
	throws IOException {
		if (!(httpReq instanceof FullTextSearchRequest)) {
			logger.warn("[doDelete] HttpServletRequest is not FullTextSearchRequest. " + httpReq.getClass().getName());
			return;
		}
		if (!(httpResp instanceof FullTextSearchResponse)) {
			logger.warn("[doDelete] HttpServletResponse is not FullTextSearchResponse. " + httpResp.getClass().getName());
			return;
		}
		FullTextSearchRequest req = (FullTextSearchRequest)httpReq;
		FullTextSearchResponse resp = (FullTextSearchResponse)httpResp;
		...
		try {
			...
			if (param.getOption(FullTextSearchRequestParam.PARAM_CLEAN) != null) {
				// BDBクリーン
				if (logger.isTraceEnabled()) {
			...
			} else if (param.getOption(FullTextSearchRequestParam.PARAM_CLOSE) != null) {
				// BDB環境クローズ
				if (logger.isTraceEnabled()) {
	 ...
	 */
	protected void doResponse(FullTextSearchRequest req, FullTextSearchResponse resp,
			Object retObj)
	throws IOException {
