...
import jp.reflexworks.taggingservice.bdb.FullTextSearchBDBConst;
import jp.reflexworks.taggingservice.bdb.FullTextSearchBDBManager;
...
import jp.reflexworks.taggingservice.redis.JedisSessionManager;
import jp.reflexworks.taggingservice.taskqueue.FullTextSearchTaskQueueUtil;
 ...
 * 全文検索コンテキスト.
 */
public class FullTextSearchContext implements BaseReflexContext {
	 ...
	 */
	public FullTextSearchContext(String serviceName, String namespace, String sid,
	 ...
	 * @return Feed
	 */
	public FeedBase getFeed(FullTextSearchRequestParam param,
			String distkeyItem, String distkeyValue)
	throws IOException, TaggingException {
		FullTextSearchBDBManager bdbManager = new FullTextSearchBDBManager();
		// id uri 一覧を取得
	 ...
	 */
	public FeedBase getCount(FullTextSearchRequestParam param,
			String distkeyItem, String distkeyValue)
	throws IOException, TaggingException {
		FullTextSearchBDBManager bdbManager = new FullTextSearchBDBManager();
		// id uri 一覧を取得
	 ...
	 * @return テーブルリスト
	 */
	public FeedBase getList(FullTextSearchRequestParam param)
	throws IOException, TaggingException {
		FullTextSearchBDBManager bdbManager = new FullTextSearchBDBManager();
		FetchInfo<?> fetchInfo = bdbManager.getList(namespace, param,
				requestInfo, connectionInfo);

		boolean isCount = param.getOption(FullTextSearchRequestParam.PARAM_COUNT) != null;
		if (isCount) {
			return returnCount(fetchInfo);
		}
		if (fetchInfo == null ||
	 ...
	 */
	public FeedBase getStats(FullTextSearchRequestParam param)
	throws IOException, TaggingException {
		FullTextSearchBDBManager bdbManager = new FullTextSearchBDBManager();
	 ...
	 */
	public void put(FeedBase feed, FullTextSearchRequestParam param)
	throws IOException, TaggingException {
		FullTextSearchBDBManager bdbManager = new FullTextSearchBDBManager();
		boolean isPartial = param.getOptionBoolean(FullTextSearchRequestParam.PARAM_PARTIAL);
		boolean isDelete = param.getOptionBoolean(FullTextSearchRequestParam.PARAM_DELETE);
	 ...
	 * @param param パラメータ
	 */
	public void cleanBDB(FullTextSearchRequestParam param)
	throws IOException, TaggingException {
		ReflexBDBCleanCallable callable = new ReflexBDBCleanCallable(
				FullTextSearchBDBConst.DB_NAMES,
				serviceName, namespace, requestInfo);
		FullTextSearchTaskQueueUtil.addTask(callable, 0,
				serviceName, namespace, null, requestInfo);
	}
	...
	public FeedBase getSessionFeed(String name) throws IOException, TaggingException {
		JedisSessionManager sessionManager = new JedisSessionManager();
		return sessionManager.getFeed(sid, name, this);
	}
	...
	throws IOException, TaggingException {
		JedisSessionManager sessionManager = new JedisSessionManager();
		return sessionManager.setFeed(sid, name, feed, this);
	}
