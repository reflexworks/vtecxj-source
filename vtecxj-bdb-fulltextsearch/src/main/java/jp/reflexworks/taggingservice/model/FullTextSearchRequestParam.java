...
import jp.reflexworks.taggingservice.servlet.FullTextSearchRequest;
 ...
 * リクエストパラメータ情報
 */
public class FullTextSearchRequestParam implements RequestType {
	...
	/** 検索条件 */
	private FullTextSearchCondition condition;
	 ...
	 * @param req リクエスト
	 */
	public FullTextSearchRequestParam(FullTextSearchRequest req) {
		// URI抽出
	 ...
	 * @return 検索条件リスト
	 */
	public FullTextSearchCondition getFullTextSearchCondition() {
		return condition;
	}
	...
	private void setCondition() {
		...
		if (!StringUtils.isBlank(queryString)) {
			...
			// 検索条件抽出
			condition = createCondition(paramList);
		}
	}
	 ...
	 * @return 検索条件
	 */
	private FullTextSearchCondition createCondition(
			List<String[]> paramList) {
		for (String[] paramPart : paramList) {
			String key = paramPart[0];
			FullTextSearchCondition ftCondition =
					FullTextSearchIndexUtil.getCondition(key);
			if (ftCondition != null) {
				return ftCondition;
			}
		}
