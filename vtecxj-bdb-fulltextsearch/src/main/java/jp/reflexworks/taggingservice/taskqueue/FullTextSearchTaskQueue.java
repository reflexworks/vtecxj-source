 ...
 * 例外発生時の処理を共通化する。
 */
public class FullTextSearchTaskQueue<T> implements ReflexTaskQueue<T> {
	 ...
	 */
	public FullTextSearchTaskQueue(ReflexBDBCallable<T> callable, String serviceName,
			String namespace, String sid, ReflexBDBRequestInfo requestInfo) {
	...
	public T call() throws IOException, TaggingException {
		...
		try {
			...
			// ReflexContext生成
			FullTextSearchContext reflexContext = new FullTextSearchContext(
					serviceName, namespace, sid, requestInfo, connectionInfo);
