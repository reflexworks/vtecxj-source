...
public class TSAClient {
	 ...
	 */
	public TimeStampToken getTimeStampToken(byte[] content) {
		digest.reset();
		...
		try {
			...
			String errMsg = e.getMessage() + " url=" + this.url;
			if (logger.isDebugEnabled()) {
				 logger.debug("[getTimeStampToken] " + errMsg, e);
			}
			throw new IllegalPdfParameterException(errMsg, e);
		}

		TimeStampToken timeStampToken = response.getTimeStampToken();
		if (timeStampToken == null) {
			// https://www.ietf.org/rfc/rfc3161.html#section-2.4.2
			throw new IllegalPdfParameterException("Response from " + url +
	...
	private byte[] getTSAResponse(byte[] request) {
		if (logger.isTraceEnabled()) {
			logger.debug("[getTSAResponse] Opening connection to TSA server");
		}
		...
		try {
			...
			String errMsg = "Exception when connecting " + this.url;
			if (logger.isDebugEnabled()) {
				 logger.debug("[getTSAResponse] " + errMsg, e);
			}
			throw new IllegalPdfParameterException(errMsg, e);
		}
		...
		if (logger.isTraceEnabled()) {
			logger.debug("[getTSAResponse] Established connection to TSA server");
		}

		if (username != null && password != null && !username.isEmpty() && !password.isEmpty()) {
			// See https://stackoverflow.com/questions/12732422/ (needs jdk8)
			// or see implementation in 3.0
			throw new IllegalPdfParameterException("authentication not implemented yet");
		}
		...
		try {
			...
			String errMsg = "Exception when writing to " + this.url;
			if (logger.isDebugEnabled()) {
				 logger.debug("[getTSAResponse] " + errMsg, ex);
			}
			throw new IllegalPdfParameterException(errMsg, ex);
		} finally {
			IOUtils.closeQuietly(output);
		}

		if (logger.isTraceEnabled()) {
			logger.debug("[getTSAResponse] Waiting for response from TSA server");
		}
		...
		try {
			...
			String errMsg = "Exception when reading from " + this.url;
			if (logger.isDebugEnabled()) {
				 logger.debug("[getTSAResponse] " + errMsg, ex);
			}
			throw new IllegalPdfParameterException(errMsg, ex);
		...
		if (logger.isTraceEnabled()) {
			logger.debug("[getTSAResponse] Received response from TSA server");
		}
