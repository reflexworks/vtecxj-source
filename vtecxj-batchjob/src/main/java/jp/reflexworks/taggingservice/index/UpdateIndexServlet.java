...
/**
 * インデックス登録・更新・削除サーブレット.
 */
public class UpdateIndexServlet extends ReflexServletBase {
	...
	throws IOException {
		...
		try {
			...
			if (param.getOption(RequestParam.PARAM_UPDATEINDEX) != null ||
				...
				if (logger.isDebugEnabled()) {
					...
					logger.debug(LogUtil.getRequestInfoStr(requestInfo) + tmpOpt);
				}
				FeedBase feed = req.getFeed();
				UpdateIndexManager updateIndexManager = new UpdateIndexManager();
				updateIndexManager.addTaskPutIndex(feed, isDelete, auth, requestInfo, connectionInfo);
