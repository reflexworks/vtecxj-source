 ...
 * インデックスのクローズ非同期処理.
 */
public class IndexCloseCallable extends ReflexCallable<Boolean> {
	 ...
	 */
	public IndexCloseCallable(String serverUrl, BDBIndexType indexType) {
	...
	public Boolean call() throws IOException, TaggingException {
		...
		if (logger.isTraceEnabled()) {
			logger.trace(LogUtil.getRequestInfoStr(requestInfo) +
					"[IndexClose call] start.");
		}

		IndexCommonManager idxManager = new IndexCommonManager();
		return idxManager.closeIndexProc(serverUrl, indexType, reflexContext);
	}
