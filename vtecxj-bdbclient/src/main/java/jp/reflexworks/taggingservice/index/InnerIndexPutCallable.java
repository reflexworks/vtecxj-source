...
public class InnerIndexPutCallable extends ReflexCallable<Boolean> {
	...
	public Boolean call() throws IOException, TaggingException {
		...
		while (true) {
			try {
				ret = idxManager.putInnerIndex(targetUpdatedInfos, reflexContext);
				break;
				...
				if (logger.isInfoEnabled()) {
					logger.info(LogUtil.getRequestInfoStr(requestInfo) +
							"[put InnerIndex after commit call] " + BDBClientUtil.getRetryLog(e, retryCount));
				}
				...
				if (logger.isWarnEnabled()) {
					logger.warn(LogUtil.getRequestInfoStr(requestInfo) +
							"[put InnerIndex after commit call] retry start. retryNo=" +
