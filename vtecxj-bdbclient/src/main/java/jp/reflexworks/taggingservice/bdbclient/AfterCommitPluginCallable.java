 ...
 * データ更新後のプラグイン非同期処理.
 */
public class AfterCommitPluginCallable extends ReflexCallable<Boolean> {

	/** 更新情報リスト */
	private List<UpdatedInfo> updatedInfos;
	/** エントリー更新後に呼び出されるプラグイン */
	private CallingAfterCommit callingAfterCommit;
	 ...
	 * @param callingAfterCommit エントリー更新後に呼び出されるプラグイン
	 */
	public AfterCommitPluginCallable(List<UpdatedInfo> updatedInfos, 
			CallingAfterCommit callingAfterCommit) {
		this.updatedInfos = updatedInfos;
		this.callingAfterCommit = callingAfterCommit;
	}
	...
	public Boolean call() throws IOException, TaggingException {
		...
		if (logger.isDebugEnabled()) {
			...
			sb.append("[after commit plugin callable] start. ");
			sb.append(" plugin = ");
			sb.append(callingAfterCommit.getClass().getName());
			sb.append(" ");
			sb.append(toStringUpdatedInfos());
			logger.debug(sb.toString());
		}

		callingAfterCommit.doAfterCommit(updatedInfos, reflexContext);

		if (logger.isTraceEnabled()) {
			StringBuilder sb = new StringBuilder();
			sb.append(LogUtil.getRequestInfoStr(requestInfo));
			sb.append("[after commit plugin callable] end. ");
			sb.append(" plugin = ");
			sb.append(callingAfterCommit.getClass().getName());
			sb.append(" ");
			sb.append(toStringUpdatedInfos());
