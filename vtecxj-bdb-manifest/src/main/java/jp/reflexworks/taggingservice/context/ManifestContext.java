...
import jp.reflexworks.taggingservice.bdb.ManifestConst;
import jp.reflexworks.taggingservice.bdb.ManifestManager;
...
import jp.reflexworks.taggingservice.taskqueue.ManifestTaskQueueUtil;
 ...
 * マニフェストコンテキスト.
 */
public class ManifestContext implements BaseReflexContext {
	 ...
	 */
	public ManifestContext(String serviceName, String namespace,
	...
	/**
	 * キー検索
	 * @param feed キーリスト <link rel="self" href={キー} />
	 * @return ID
	 */
	public FeedBase getId(String uri)
	 ...
	 * @return IDリスト
	 */
	public FeedBase getIds(FeedBase feed)
	throws IOException, TaggingException {
		ManifestManager bdbManager = new ManifestManager();
		return bdbManager.getEntryKeys(namespace, feed, serviceName,
	 ...
	 * Feed検索.
	 * @param serviceName サービス名
	 * @param param URIと検索条件(URL前方一致かどうか)
	 * @return Feed
	 */
	public FeedBase getFeed(ManifestRequestParam param)
	throws IOException, TaggingException {
		ManifestManager bdbManager = new ManifestManager();
		// id uri 一覧を取得
		FetchInfo<String> fetchInfo = bdbManager.getFeedKeys(namespace, param, this);
		if (fetchInfo == null ||
	 ...
	 */
	public FeedBase getCount(ManifestRequestParam param)
	throws IOException, TaggingException {
		ManifestManager bdbManager = new ManifestManager();
		// id uri 一覧を取得
		FetchInfo<String> fetchInfo = bdbManager.getFeedKeys(namespace, param, this);
	 ...
	 * @return テーブルリスト
	 */
	public FeedBase getList(ManifestRequestParam param)
	throws IOException, TaggingException {
		ManifestManager bdbManager = new ManifestManager();
		FetchInfo<?> fetchInfo = bdbManager.getList(namespace, param,
				requestInfo, connectionInfo);

		boolean isCount = param.getOption(ManifestRequestParam.PARAM_COUNT) != null;
		if (isCount) {
			return returnCount(fetchInfo);
		}
		if (fetchInfo == null ||
	 ...
	 */
	public FeedBase getStats(ManifestRequestParam param)
	throws IOException, TaggingException {
		ManifestManager bdbManager = new ManifestManager();
	 ...
	 * @param feed Feed
	 */
	public void put(FeedBase feed)
	throws IOException, TaggingException {
		ManifestManager bdbManager = new ManifestManager();
		bdbManager.put(namespace, feed, serviceName, requestInfo, connectionInfo);
	}
	 ...
	 * @param param パラメータ
	 */
	public void cleanBDB(ManifestRequestParam param)
	throws IOException, TaggingException {
		ReflexBDBCleanCallable callable = new ReflexBDBCleanCallable(
				ManifestConst.DB_NAMES,
				serviceName, namespace, requestInfo);
		ManifestTaskQueueUtil.addTask(callable, 0,
				serviceName, namespace, requestInfo);
	}
