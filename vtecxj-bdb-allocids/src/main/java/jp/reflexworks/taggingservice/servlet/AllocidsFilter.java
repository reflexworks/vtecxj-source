 ...
 * BDBマニフェストサーブレットフィルタ
 */
public class AllocidsFilter implements Filter, ReflexServletConst {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		// リクエスト・レスポンスをラップ
		AllocidsRequest req = new AllocidsRequest(httpReq);
		AllocidsResponse resp = new AllocidsResponse(req, httpResp);

		RequestInfo requestInfo = req.getRequestInfo();

		try {
			if (!BDBEnvUtil.isRunning()) {
				throw new IllegalStateException("Tagging BDB service (manifest) is not running.");
			}
	 ...
	 * @param req リクエスト
	 */
	private void checkRequest(AllocidsRequest req) {
	...
	throws IOException {
		...
		// スタックトレースをログ出力
		if (logger.isInfoEnabled()) {
			// NoEntryException、OutOfRangeException、IllegalParameterException以外を出力
			if (!(e instanceof NoEntryException) && !(e instanceof OutOfRangeException) &&
					 !(e instanceof IllegalParameterException)) {
