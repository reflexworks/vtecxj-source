 ...
 * GET Feedバッチ.
 */
public class GetFeedBlogic implements ReflexBlogic<ReflexContext, FeedBase> {
	...
	public FeedBase exec(ReflexContext reflexContext, String[] args) {
		...
		if (args.length < 1) {
			throw new IllegalStateException("引数が不足しています。[0]リクエストURI、[1]useCache ");
		}
		...
		try {
			...
			if (useCache != null) {
				...
				feed = systemContext.getFeed(requestUri);
			}
